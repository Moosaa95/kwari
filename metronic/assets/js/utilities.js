...
function generateExcel(data, sheetName, fileName) {
	...
	for (var i = 0; i < binaryWb.length; i++)
		view[i] = binaryWb.charCodeAt(i) & 0xff; //convert to octet
...
function formatData(
	...
	const newDataList = filteredData.reduce(function (acc, elem) {
		$.each(elem, function (key, value) {
			...
			elem['is_reversal'] = !!(
				key === isReversalCol && isReversal.includes(value)
			);
			delete elem[key];
		});
		acc.push(elem);
...
function closeAgentCredentials() {
	notifier.style.animation = 'slide-out 1s ease-in backwards';
	setTimeout(() => {
		$('#notifier').hide();
	}, 1500);
}

function showAgentCredentials() {
	notifier.style.display = 'block';
	notifier.style.animation = 'slide-in 1s ease-in forwards';
}

$('#close-notifier').on('click', function () {
	closeAgentCredentials();
});
